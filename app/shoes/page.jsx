...
function Shoes() {
  ...
  const handleUserClick = (postId) => {
    ...
    const newURL = `http://localhost:3000/shoes/${postId}`;
    window.history.pushState(null, '', newURL);
  };

  useEffect(() => {
    const handlePopstate = () => {
      const postId = window.location.pathname.replace('/shoes/', '');
      setBlockId(postId);
    };
  ...
  const addToCart = () => {
    if (block) {
      const updatedCartItems = [...cartItems, block]; // Добавляем новый товар к существующим товарам в корзине
      setCartItems(updatedCartItems); // Обновляем состояние cartItems
      localStorage.setItem('cartItems', JSON.stringify(updatedCartItems)); // Сохраняем обновленные товары в localStorage
    }
  };
        ...
        {block && (
          <div className="clothing_block">
            ...
            <div className="clothing_block-box clothing_block_center">
              ...
              <>
                <i className="fa-solid fa-heart" style={blockStyle} onClick={handleClick}></i>
              </>
            </div>
            <div className="clothing_block-box">
